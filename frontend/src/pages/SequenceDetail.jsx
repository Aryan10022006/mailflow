...
function getSendStatus(send) {
    ...
    return (
        SEND_STATUS[send.status] || { label: send.status, cls: "badge-draft" }
    );
}
...
export default function SequenceDetail() {
    ...
    const resumeContact = async (contactId, email) => {
        try {
            const res = await api.post(
                `/sequences/${id}/contacts/${contactId}/resume`,
            );
            toast.success(
                res.data.message ||
                    `Resumed ${email} from step ${res.data.nextStep}`,
            );
            load();
        } catch (err) {
            toast.error(
                err.response?.data?.error || "Failed to resume contact",
            );
        }
    };
    ...
    const selectAllScheduled = () => {
        const scheduledIds = filteredContacts
            .filter((c) =>
                (c.sends || []).some((s) => s.status === "scheduled"),
            )
            .map((c) => c.id);
    ...
    const executeBulkAction = async () => {
        ...
        if (bulkAction === "reschedule" && !bulkDateTime)
        ...
        (async () => {
            ...
            for (const contactId of contactsToProcess) {
                ...
                if (!contact) continue;
                const failedOrScheduled = (contact.sends || []).filter(
                    (s) => s.status === "failed" || s.status === "scheduled",
                );
            ...
            toast.success(
                `✅ Done! ${success} succeeded${failed > 0 ? `, ${failed} failed` : ""}`,
            );
            load();
        })();
    };
        ...
        <div className="main-content">
            ...
            <div className="page-body">
                ...
                {seq.daily_limit_hit && (
                    ...
                    >
                        ⚠️ Gmail daily sending limit reached. Sending will
                        resume at{" "}
                        {seq.daily_limit_reset_at
                            ? formatIST(seq.daily_limit_reset_at)
                            : "tomorrow"}
                        .
                    </div>
                )}
                ...
                <div className="stats-grid" style={{ marginBottom: 24 }}>
                    {[
                        ...
                        {
                            label: "Sent",
                            value: seq.sent_count || 0,
                            color: "var(--green)",
                        },
                        {
                            label: "Opened",
                            value: seq.opened_count || 0,
                            color: "var(--yellow)",
                        },
                        {
                            label: "Replied",
                                            ...
                                            {filteredContacts.map((c) => {
                                                ...
                                                return (
                                                    <tr key={c.id}>
                                                        ...
                                                        <td>
                                                            <span
                                                                className={`badge ${STATUS_COLORS[c.status] || "badge-draft"}`}
                                                            >
                                                                {c.status}
                                                            </span>
                                                        </td>
                        ...
                        {activity.length === 0 ? (
                            <div className="empty-state">
                                ...
                                <div className="empty-state-desc">
                                    Activity will appear here once the sequence
                                    is launched
                                </div>
                            </div>
                        ) : (
                    ...
                    <div className="modal" onClick={(e) => e.stopPropagation()}>
                        <div className="modal-header">
                            <div className="modal-title">
                                ⚡ Bulk Action — {selectedContacts.size}{" "}
                                contacts
                            </div>
                            <button
                        ...
                        <div className="modal-body">
                            ...
                            <div className="form-group">
                                ...
                                <div style={{ display: "flex", gap: 8 }}>
                                    <button
                                        className={`btn btn-sm ${bulkAction === "reschedule" ? "btn-primary" : "btn-secondary"}`}
                                        onClick={() =>
                                    ...
                                    <button
                                        className={`btn btn-sm ${bulkAction === "send_now" ? "btn-primary" : "btn-secondary"}`}
                                        onClick={() =>
                                            setBulkAction("send_now")
                                        }
                                    >
                                        ⚡ Send Now
                                    </button>
                                ...
                                <>
                                    <div className="form-group">
                                        ...
                                        <input
                                            ...
                                            value={bulkDateTime}
                                            onChange={(e) =>
                                                setBulkDateTime(e.target.value)
                                            }
                                        />
                                    </div>
                                    <div className="form-group">
                            ...
                            {bulkAction === "send_now" && (
                                <div className="form-group">
                                    <label className="form-label">
                                        Send From (optional — uses sequence
                                        default if empty)
                                    </label>
                                    <select
                                        className="input"
                            ...
                            >
                                This will apply to all failed and scheduled
                                emails for the {selectedContacts.size} selected
                                contact(s).
                            </div>
                        </div>
            ...
            {tooltip && (
                ...
                >
                    ...
                    >
                        ...
                        >
                            {tooltip.send.status === "scheduled"
                                ? "🕐 Scheduled"
                                : tooltip.send.status === "sent"
                                  ? "✅ Sent"
                                  : tooltip.send.status}
                        </span>
                    </div>
                    ...
                    >
                        ...
                        >
                            {new Date(
                                (
                                    tooltip.send.sent_at ||
                                    tooltip.send.scheduled_at ||
                                    ""
                                )
                                    .replace("Z", "")
                                    .replace(/[+-]\d{2}:\d{2}$/, ""),
